...
INSERT INTO fms.email_template (uid,name,sender,recipients,subject,template) VALUES
    ('5560da6f-cc02-4d8b-92df-6ee550142ff5','AppointmentBooking','devops@parkwaypantai.com','devops@parkwaypantai.com','Appointment Booking','Submission Details:<br/><br/>Source URL: ${sourceUrl}<br/><br/>Appointment Details:<br/><br/>Preferred Doctor: ${preferredDoctor}<br/>Specialty: ${specialty}<br/>Preferred Hospital: ${hospitalSource}<br/>Preferred Appointment Date: ${preferredDate}<br/>Preferred Timeslot: ${preferredTimeslot}<br/>Reason For Consultation: ${reason}<br/><br/>Patient Information:<br/><br/>Name: ${patientFullName}<br/>Nationality: ${patientNationality}<br/>Country: ${patientCountry}<br/>Nearest Office: ${nearestOffice}<br/>Gender: ${patientGender}<br/>Date of birth: ${patientDob}<br/>NRIC/Passport number: ${patientIdValue}<br/>Email Address: ${patientEmail}<br/>Contact No.: ${patientContact}<br/><br/>Additional Information:<br/><br/>Receive Marketing Messages?: ${receiveMarketing}<br/>Requested For: ${bookingFor}<br/>Requestor: ${caregiverFullName}<br/>Requestor Email: ${caregiverEmail}<br/>Requestor Country: ${caregiverCountry}<br/>Requestor Contact No.: ${caregiverContact}');
